...
void ColocaPilhaBitMap(bitmap *bitm, Pilha* pilha) {
    ...
    int lenght = bitmapGetLength(*bitm);
    int max = bitmapGetMaxSize(*bitm);

    int tam = max - lenght;

    for (aux = 0; aux < tam; aux++) {

        bitmapAppendLeastSignificantBit(bitm, rtnCaracterPilha(pilha, aux));
        
        if (rtnTopoPilha(pilha) - 1 == aux) {
...
void EscreveBitmap(FILE* compacta, bitmap * bitm, int topo) {
    ...
    unsigned char*  vet = (unsigned char*)bitmapGetContents(*bitm);

    for (aux = 0; aux < tamf; aux++) {

        fprintf(compacta, "%c", vet[aux]);
